...
public class Trafic {
	...
	public Trafic() {
		...
		for (int i=1; i<1000; i++) {
			...
			if (randfile==0) {
				if (randvehi<0.3) {vehicule =new Moto(125000-250*i,0);}
				else if (randvehi < 1.5) {vehicule= new Voiture(125000-250*i,0);}
				else {vehicule =new Camion(125000-250*i,0);}
				listev1.add(vehicule);	
			}
			else if (randfile==1) {
				if (125000-250*i < 800 || 125000-250*i > 1100) {
					if (randvehi<1.15) {vehicule =new Moto(125000-250*i,1);}
					else if (randvehi < 2.3) {vehicule =new Voiture(125000-250*i,1);}
					else {vehicule =new Camion(125000-250*i,1);}
					listev2.add(vehicule);
				}
			}
			else {
				if (randvehi<1.48) {vehicule =new Moto(125000-250*i,2);}
				else if (randvehi < 2.96) {vehicule =new Voiture(125000-250*i,2);}
				else {vehicule =new Camion(125000-250*i,2);}
	...
	public void freinage(Entites A, Entites B) {
		...
		if (B.getxPos()-A.getxPos()< 1.05*Ressources.Constantes.HAUTEUR_CAMION || A.getDy()>B.getDy()) /*freinage d'urgence*/ {
			A.setDy(A.getDy()-2);
		} else if (B.getxPos()-A.getxPos()< 1.2*Ressources.Constantes.HAUTEUR_CAMION || A.getDy()>B.getDy())/*freinage normal*/ {
			A.setDy(A.getDy()-1);
		} else if (B.getxPos()-A.getxPos()> 3*Ressources.Constantes.HAUTEUR_CAMION && A.getDy()<A.getDyinit()) {
			A.setDy(A.getDy()+1); // Interessant si on pouvait mettre +0.2 ou alors augmenter échelle des possibilités de vitesse
		}
	...
	public boolean depassement(Entites A, Entites B) {
		...
		if (A.getDy()-B.getDy()> 20 || randdep<1) {
