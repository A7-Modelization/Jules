...
public class Trafic {
	...
	public void freinage(Entites A, Entites B) {
		...
		if (B.getxPos()-A.getxPos()< 1.05*Ressources.Constantes.HAUTEUR_CAMION || A.getDx()>B.getDx()) /*freinage d'urgence*/ {
			A.setDx(A.getDx()-2);
		} else if (B.getxPos()-A.getxPos()< 1.2*Ressources.Constantes.HAUTEUR_CAMION || A.getDx()>B.getDx())/*freinage normal*/ {
			A.setDx(A.getDx()-1);
		} else if (B.getxPos()-A.getxPos()> 3*Ressources.Constantes.HAUTEUR_CAMION && A.getDx()<A.getDxinit()) {
	...
	public void changement_voie(LinkedList<Entites> L1, LinkedList<Entites>)
