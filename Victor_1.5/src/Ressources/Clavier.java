...
public class Clavier implements KeyListener{
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_UP) {
			Constantes.VITESSE_RELATIVE=Constantes.VITESSE_RELATIVE +1;
			int longueur=Main1.scene.trafic.tabVoitures.size();
			for (int i=0; i<longueur; i++) {
				Main1.scene.trafic.tabVoitures.get(i).setDy(Main1.scene.trafic.tabVoitures.get(i).getDy()+1);}
		}
		else if (e.getKeyCode() == KeyEvent.VK_DOWN) {
			if (Constantes.VITESSE_RELATIVE >1) {Constantes.VITESSE_RELATIVE=Constantes.VITESSE_RELATIVE -1;
			int longueur=Main1.scene.trafic.tabVoitures.size();
			for (int i=0; i<longueur; i++) {
				Main1.scene.trafic.tabVoitures.get(i).setDy(Main1.scene.trafic.tabVoitures.get(i).getDy()-1);}
			}
